	...
	public $rules = [
		...
		'autospace_after_dot' => [
				...
				'replacement' 	=> [
						//'\1\2. \4',
						'$m[1].$m[2]."." .( $m[5] == "." ? "" : " ").$m[4].$m[5]',
						'$m[1].$m[2]."." .([EMT_Lib::strtolower($m[3]), $this->domain_zones]? "":( $m[4] == "." ? "" : " ")). $m[3].$m[4]'
						],
			],
    	...
    	'nbsp_before_pretext' => [
      			...
      			'pattern' 		=> '/([a-zA-Zа-яёА-ЯЁ])(\040|\t|\&#8232;)+(без|в|для|до|за|из|из-за|из-под|к|ко|на|над|о|об|от|перед|по|под|пред|при|про|с|со|у|через)([^\<]|$)/iu', 
				'replacement' 	=> '\1&nbsp;\3\4'
      		],
    	'nbsp_after_numbers' => [
      			'description'	=> 'Неразрывный пробел после чисел',
      			'pattern' 		=> '/(\d+)(\040|\t|\&#8232;)+([a-zA-Zа-яёА-ЯЁ]+)([^\<]|$)/iu', 
				'replacement' 	=> '\1&nbsp;\3\4'
      		],
		'nbsp_before_month'	=> [
